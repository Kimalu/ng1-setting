import TableConfig from '../model/jenkins_http';
...
/**/
export default class JenkinsHttpCtrl extends monitoringCtrl{
	...
	rerender(data) {
			...
			this.itemList = (data.content || []).map(item => {
					...
					item.searchConsole = API.search.format({
							host: item.request_host.split('://')[1],
...
JenkinsHttpCtrl.$inject = ['commonJenkinsService', 'appDialog', '$scope'];
